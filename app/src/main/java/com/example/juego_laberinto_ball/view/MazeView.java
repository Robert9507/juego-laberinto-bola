package com.example.juego_laberinto_ball.view;
...
import com.example.juego_laberinto_ball.misc.PaintStyles;
...
//    sensor controls
    private void updateBall(SensorEvent event) {
        ...
        // Aquí realizamos la detección de colisiones con los muros del laberinto
        float nextXBall = this.xBall + xBallDelta;
        float nextYBall = this.yBall + yBallDelta;
        if (checkWallCollision(nextXBall, nextYBall)) {
            // Si hay colisión con un muro, ajustamos la velocidad de la pelota o detenemos su movimiento según corresponda
            xBallDelta = 0; // Por ejemplo, detenemos el movimiento horizontal
            yBallDelta = 0; // Y también el movimiento vertical
        }
    ...
    private boolean checkWallCollision(float nextX, float nextY) {
        int nextLine = (int) pxToLine(nextY);
        int nextCol = (int) pxToCol(nextX);
        return maze[nextLine][nextCol] > 0;
    }

    private void gameOver() {
        this.finished = true;
        onPause();
        // Aquí se puede mostrar un mensaje de "game over" o realizar cualquier acción adicional.
        // Por ejemplo:
        // Toast.makeText(getContext(), "Game Over!", Toast.LENGTH_SHORT).show();
        // O iniciar otra actividad que muestre un mensaje de "game over" y permita reiniciar el juego.
    }
...
//    draw labirint
    ...
    private void drawInnerSquares(int line, int col, Canvas canvas, Paint paint) {
        ...
        //float radius = Math.min(stopX - startX, stopY - startY) * 0.2f; // Define el radio para las esquinas redondeadas

        //RectF rect = new RectF(startX, startY, stopX, stopY);
        RectF mRectF = new RectF(startX, startY, stopX, stopY);
        canvas.drawRect(mRectF, paint);
        //canvas.drawRoundRect(rect, radius, radius, paint); // Dibuja un rectángulo redondeado
    }
    ...
    private void drawLevelNumber(Canvas canvas) {
        ...
        canvas.drawText("Nivel. " + this.currentLevel, 10, 60, paint);
    }
}
